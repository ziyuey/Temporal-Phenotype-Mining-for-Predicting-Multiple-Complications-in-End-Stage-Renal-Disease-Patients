...
def plot_attention(alpha, beta, gamma, clus_pred=None, feats=None):
    ...
    shunxu_list = [10,4,13,1,8,6,12,2,11,0,7,3,5,9,]
    def min_max_(matrix):
    ...
    for k in range(K):
        ...
        heatmap_k_1 = min_max_(heatmap_k_1)
        heatmap_k_2 = min_max_(heatmap_k_2)
        ...
        sns.heatmap(data=heatmap_k_1, ax=axes1[k], cbar=k == 0,cbar_ax=None if k else cbar_ax1,cmap='plasma')
        sns.heatmap(data=heatmap_k_2, ax=axes2[k], cbar=k == 0,cbar_ax=None if k else cbar_ax2,cmap='plasma')

        # Add Titles
        axes1[shunxu_list[k]].set_title(f"Clus {k}")
        axes2[shunxu_list[k]].set_title(f"Clus {k}")
...
def _get_attention_v1(alpha, beta, gamma_k):
    ...
    return np.mean(cohort_attention, axis=0).T
...
def get_basic_info(save_fd: str = None, data_info: dict = None):
    ...
    if save_fd is not None:
        ...
        print("get_basic_info save_fd",save_fd)
        # _, model_data_used, model_name, run_num, _ = save_fd.split("/")
        # assert data_name == model_data_used
        model_name, run_num, _, _ = save_fd.split("/")

        # Update save_fd
        save_fd = f"{model_name}/{run_num}/visualisations/"
