...
def visualise_cluster_groups(data_info: dict, save_fd: str, cluster_pred=None, **kwargs):
    ...
    if cluster_pred is not None:
        ...
        ids_test, id_col = data_info["ids"][-1], data_properties["id_col"]
...
def visualise_cluster_assignment(data_info, clus_pred=None, pis_pred=None, save_fd=None, **kwargs):
    ...
    if clus_pred is not None:
        ...
        # Get y_test
        ## Note:修改
        print("data_info  y_test,", np.shape(data_info["y"][-1][:,0:-2]))
        print("data_info  y_test,",data_info["y"][-1][:,0:-2])
        labels_true = np.argmax(data_info["y"][-1][:,0:-2], axis=1)
        print("classes of labels_true,", set(labels_true))
        print("shape of labels_true",np.shape(labels_true))
        ...
        print("shape of clus_pred", np.shape(clus_pred))

        # Compute contingency matrix
        cont_matrix_temp = cont_matrix(labels_true=labels_true, labels_pred=clus_pred)
        print("res of cont_matrix_temp,",cont_matrix_temp)

        cm = pd.DataFrame(cont_matrix_temp,
...
def visualise_attention_maps(save_fd=None, data_info: dict = None, clus_pred=None, **kwargs):
    ...
    if clus_pred is not None:
        ...
        for key, values in kwargs.items():
            print("vis attention map key:",key)
            print("vis attention map values:", values)
            if ("attention" in key or "att" in key) and ("unnorm" not in key):
                ...
                # Save if save_fd
                if save_fd is not None:
                    print("Start saving plot_attention")
                    fig1.savefig(save_fd + "Cluster-wise attention with gamma_norm.jpeg", dpi=500)
                    fig2.savefig(save_fd + "Cluster-wise attention with sub-cohort_norm.jpeg", dpi=500)
            if ("attention" in key or "att" in key) and ("unnorm" in key):
                ...
                if save_fd is not None:
                    print("Start saving plot_attention")
                    fig1.savefig(save_fd + "Cluster-wise attention with gamma_unnorm.jpeg", dpi=500)
                    fig2.savefig(save_fd + "Cluster-wise attention with sub-cohort_unnorm.jpeg", dpi=500)
