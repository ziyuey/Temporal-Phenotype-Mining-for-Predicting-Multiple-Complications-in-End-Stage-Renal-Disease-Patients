...
class CAMELOT(tf.keras.Model):
    ...
    def _initialise_clus(self, x, val_x, **kwargs):
        ...
        batch_size = 1024
        for iter in range(int(len(x) / batch_size)):
            batch_x = BatchNext(x, batch_size, iter, val_idx)
        ...
        def BatchNext(x_train, batch_size, iters, idx):
            ...
            return batch_x
        z_val = []
        val_idx = np.arange(0, val_x.shape[0])
        batch_size = 1024
        for iter in range(int(len(val_x) / batch_size)):
            batch_x = BatchNext(val_x, batch_size, iter, val_idx)
            z_val_batch = self.Encoder(batch_x).numpy()
            z_val.extend(z_val_batch)
