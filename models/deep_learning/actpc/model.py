...
from src.models.deep_learning.model_blocks import MLP, LSTMEncoder,multi_task_MLP
...
class ACTPC(tf.keras.Model):
    ...
    def __init__(self, num_clusters=10, latent_dim=32, seed=4347, output_dim=4, name="CAMELOT",
        ...
        self.Critic = MLP(output_dim=self.output_dim, dropout=self.dropout, output_fn="sigmoid",
                          regulariser_params=self.regulariser, seed=self.seed, name="Critic",
                          **self.Critic_params)
        # self.Critic = multi_task_MLP(output_dim=self.output_dim, dropout=self.dropout, output_fn="sigmoid",
        #                                 regulariser_params=self.regulariser, seed=self.seed, name="Critic",
        #                                 **self.Critic_params)
        ...
        for epoch in range(epochs):  # Iterate through epochs
            ...
            for step_, (x_batch, clus_batch) in enumerate(input_dataset):  # Iterate through batch
                ...
                with tf.GradientTape(watch_accessed_variables=False) as tape:
                    ...
                    loss_batch = model_utils.l_crit_init(clus_batch, clus_pred)
            ...
            for val_step_, (x_batch, clus_batch) in enumerate(val_dataset):
                ...
                loss_val_batch = model_utils.l_crit_init(clus_batch, clus_val_pred)
...
class Model(ACTPC):
    ...
    def analyse(self, data_info):
        ...
        y_pred = pd.DataFrame(output_test, index=pat_ids, columns=outc_dims + ['E'])
