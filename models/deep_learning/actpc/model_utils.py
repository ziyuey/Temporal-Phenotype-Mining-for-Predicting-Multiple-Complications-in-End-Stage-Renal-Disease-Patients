...
# ------------------------------------------------------------------------------------
"""Loss Functions"""
def l_crit_init(y_true, y_pred, name='pred_clus_init_loss'):
    """
...
def l_crit(y_true, y_pred, name='pred_clus_loss'):
    ...
    # Note:可以加权重
    y_pred_crit = tf.clip_by_value(y_pred, tf.pow(0.1, 7), 0.9999999)
    ...
    loss_value = tf.reduce_mean(tf.reduce_mean(loss))
