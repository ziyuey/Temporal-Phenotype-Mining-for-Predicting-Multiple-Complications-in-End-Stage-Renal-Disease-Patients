...
class MLP(Layer):
    ...
    def __init__(self, output_dim: int, hidden_layers: int = 2, hidden_nodes: int = 30, activation_fn='relu',
...
class multi_task_MLP(Layer):
    ...
    def __init__(self, output_dim: int, hidden_layers: int = 2, hidden_nodes: int = 30, activation_fn='relu',
        ...
        # Input and Output layers
        self.output_layer_pred = Dense(units=self.output_dim, activation=self.output_fn)
        self.output_layer_survival = Dense(units=1, activation=None)
    ...
    def call(self, inputs, training=True, **kwargs):
        ...
        output_pred = self.output_layer_pred(x_inter, training=training)
        output_surv = self.output_layer_survival(x_inter, training=training)
        concatenated_outputs = tf.concat([output_pred,output_surv],axis=1)
...
class FeatTimeAttention(Layer):
    ...
    def compute_o_hat_and_alpha(self, x, latent_reps):
        ...
        # print("self.kernel.dtype",self.kernel.dtype)
        # print("tf.cast(self.kernel,tf.float32).dtype", tf.cast(self.kernel,tf.float32).dtype)
        # print("tf.expand_dims(x, axis=-1).dtype", tf.expand_dims(x, axis=-1).dtype)
        feature_projections = self.activation(tf.math.multiply(
            tf.cast(tf.expand_dims(x, axis=-1),tf.float64),
            tf.cast(self.kernel,tf.float64)) +
                                              tf.cast(self.bias,tf.float64))
        feature_projections = tf.cast(feature_projections,tf.float32)
